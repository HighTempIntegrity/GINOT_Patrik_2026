      ...
      DIMENSION TIME(2)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
      REAL*8 PE1(345320)                               ! Size of PEMatrix matrix
      common /PE1matrix/ PE1                         ! Consider PEMatrix as a common 
      REAL*8 PE2(345320)                               ! Size of PEMatrix matrix
      common /PE2matrix/ PE2                         ! Consider PEMatrix as a common variable
      REAL*8 PE3(345320)                               ! Size of PEMatrix matrix
      common /PE3matrix/ PE3                         ! Consider PEMatrix as a common variable
      REAL*8 PE12(345320)                               ! Size of PEMatrix matrix
      common /PE12matrix/ PE12                         ! Consider PEMatrix as a common variable
      REAL*8 PE13(345320)                               ! Size of PEMatrix matrix
      common /PE13matrix/ PE13                         ! Consider PEMatrix as a common variable
      REAL*8 PE23(345320)                               ! Size of PEMatrix matrix
      common /PE23matrix/ PE23                         ! Consider PEMatrix as a common variable
	  REAL*8 ELEMENTNR(345320)
	  common /ELEMENTNRmatrix/ ELEMENTNR
	  REAL*8 IPNR(345320)
	  common /IPNRmatrix/ IPNR
	  IF (LOP==0) THEN                                ! Read the txt file only only once
          OPEN(UNIT=1, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_11.txt") 
              READ(1,*) PE1
          CLOSE (1)
          OPEN(UNIT=2, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_22.txt") 
              READ(2,*) PE2
          CLOSE (2)
          OPEN(UNIT=3, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_33.txt") 
              READ(3,*) PE3
          CLOSE (3)
          OPEN(UNIT=4, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_12.txt") 
              READ(4,*) PE12
          CLOSE (4)
          OPEN(UNIT=5, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_13.txt") 
              READ(5,*) PE13
          CLOSE (5)
          OPEN(UNIT=9, FILE="/cluster/scratch/mpatrik/UEXPAN/Eigenstrains_Batch4_23.txt") 
              READ(9,*) PE23
          CLOSE (9)
		  OPEN(UNIT=11, FILE="/cluster/scratch/mpatrik/UEXPAN/CL60_Element.txt") 
              READ(11,*) ELEMENTNR
          CLOSE (11)
		  OPEN(UNIT=12, FILE="/cluster/scratch/mpatrik/UEXPAN/CL60_IP.txt") 
              READ(12,*) IPNR
          CLOSE (12)
      END IF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
